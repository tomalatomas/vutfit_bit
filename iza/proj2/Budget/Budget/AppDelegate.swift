...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func preloadData(){
        ...
        if userDefaults.bool(forKey: preloadedDataKey) != true{
            ...
            // Insert not specified category
            let context = (UIApplication.shared.delegate as! AppDelegate).persistentContainer.viewContext
            let newCategory = Category(context: context)
            newCategory.name = "Not Specified"
    
            // Insert monthly budget

            let monthlyBudgetKey = "budgetMonth"
            userDefaults.setValue(0, forKey: monthlyBudgetKey)
            
            let currencyKey = "currency"
            userDefaults.setValue("CZK", forKey: currencyKey)
            //Save
            do{
                try MOC().save()
            } catch{
