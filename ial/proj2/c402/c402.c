...
**/
    tBTNodePtr where;  // Position to insert node
    ...
    if (!found) {  //Node doesnt exist yet
        tBTNodePtr newPtr = malloc(sizeof(struct tBTNode));
        if (newPtr != NULL) {  // Malloc succesful
            newPtr->Cont = Content;
            newPtr->RPtr = NULL;
            newPtr->LPtr = NULL;
            if (where == NULL) {  //new pointer is root
                *RootPtr = newPtr;
            } else if (where->Cont > Content) {
                where->LPtr = newPtr;  //New pointer is in left pointer
            } else {
                where->RPtr = newPtr;  //New pointer is in right subtree
            }
        }
    }
}
...
**/
    tStackP s1;
    SInitP(&s1);
    Leftmost_Preorder(RootPtr, &s1);
    while (!SEmptyP(&s1)) {
        RootPtr = STopPopP(&s1);
        Leftmost_Preorder(RootPtr->RPtr, &s1);
    }
}
...
**/
    ...
    tStackB stack_bol;
    SInitB(&stack_bol);
    Leftmost_Postorder(RootPtr, &stack_ptr, &stack_bol);
    while (!SEmptyP(&stack_ptr)) {
        RootPtr = STopPopP(&stack_ptr);
        SPushP(&stack_ptr, RootPtr);  // vraceni uzlu do zasobniku
        zleva = STopPopB(&stack_bol);
        if (zleva) {
            SPushB(&stack_bol, false);
            Leftmost_Postorder(RootPtr->RPtr, &stack_ptr, &stack_bol);
        } else {
            STopPopP(&stack_ptr);
            BTWorkOut(RootPtr);
        }
