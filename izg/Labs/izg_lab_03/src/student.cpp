...
void pinedaPolygon(const Point *points, const int size, const RGBA &color1, const RGBA &color2) {
    ...
    //1. Nalezení 𝑥𝑚𝑖𝑛, 𝑥𝑚𝑎𝑥, 𝑦𝑚𝑖𝑛 a 𝑦𝑚𝑖𝑛 zadaného polygonu a oříznutí podle velikosti okna.
    int minX = points[0].x;
    int maxX = points[0].x;
    int minY = points[0].y;
    int maxY = points[0].y;
    ...
    {
        //Prochazeni bodama a hledani max min
    ...
    //2. Pro každou hranu 𝑒𝑖 výpočet ∆𝑥𝑖, ∆𝑦𝑖 (souřadnice vektoru).
    for (int i = 0; i < size; i++) {
        if (i == size-1) {
    ...
    //Pro každou hranu 𝑒𝑖 polygonu inicializuj 𝐸𝑖(𝑥𝑚𝑖𝑛, 𝑦𝑚𝑖𝑛).
    ...
    //3. Kontrola konvexnosti zadaného polygonu.
    //////// DOPLNTE KOD /////////
    ...
    for (int y = minY; y <= maxY; y++) {
        ...
        int stepX = even ? 1 : -1;
        for (int x = startX; x != endX; x += stepX) {
            //Je-li 𝐸𝑖(𝑥, 𝑦) ≥ 0 pro všechny hrany, pak nastav hodnotu pixelu. 
            bool overzero = true;
            for (int i = 0; i < size; i++){ 
                if (edgeFncVal[i] < 0){
                    //Nejaka hranova funkce hrany je minusova - nebudeme vykreslovat
                    overzero = false;
                }
            }
            if (overzero) {
                //Hranove funkce jsou vsechny kladne, muzeme dat pixe
                putPixel(x, y, color1);
            }
            if (x != endX - stepX) {
                for (int i = 0; i < size; i++) {
